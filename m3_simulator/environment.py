...
class Environment:
    ...
    def __init__(self, num_regions: int = 20, seed: int = 42):
        ...
        # Store current scenario config
        self.current_config = None

        # For S7 sudden appearance
    ...
    def step(self, action: str) -> Tuple[Dict[str, Any], float, bool, Dict[str, Any]]:
        ...
        Args:
            action: Region ID to scan (e.g., 'R7')
        ...
        # Apply switching cost if changing regions
        ...
        # Check if enough budget for this scan
        if self.budget_remaining < actual_cost:
            self.done = True
            return (
                self.get_observation_state(),
                0,
                True,
    ...
    def _calculate_reward(self, scan_result: Optional[ScanResult]) -> float:
        ...
        # Simple reward: detection success - cost penalty
        reward = 0.0
        if scan_result.detected:
            # Positive reward for detection
            reward += scan_result.confidence * 0.5
            # Extra reward for high threat
    ...
    def get_observation_state(self) -> Dict[str, Any]:
        ...
        # Build observation for each region
        regions = []
        for i in range(1, self.num_regions + 1):
            region_id = f"R{i}"
            truth = hidden.get(region_id, {})

            # Check if this region has been scanned recently
            last_scan = None
            ...
            if last_scan:
                # Region has been scanned - show observation
                regions.append(
                    {
                        "region_id": region_id,
                ...
                # Region not scanned - high uncertainty
                regions.append(
                    {
                        "region_id": region_id,
                        "existence": 0.0,
        ...
        # Count intelligence stats
        detected_count = sum(1 for r in regions if r.get("detected", False))
        high_priority_count = sum(
            1 for r in regions if r.get("threat_relevance", 0) > 0.7
        )
